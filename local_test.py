...
col = [list(map(int, _.split(" "))) for _ in col_text.split("\n")]
row = [list(map(int, _.split(" "))) for _ in row_text.split("\n")]
cal_pmap(row, True)
cal_pmap(col, False)
epochs = 20
...
# print_lim_map(p_col_map)
print()
# print_lim_map(p_row_map)
print_map(amap)
# print()

# 先左极右极确定位置，后融合，后重新计算probe图，确定x，再左极右极确定位置，迭代下去
